...
function buildModel(findings, { scope, meta }) {
  ...
  const resolved = live.filter((f) => ['fixed', 'accepted-risk', 'retest-pending'].includes(f.status));
...
function renderMarkdown(model) {
  ...
  if (model.resolved.length) {
    p(`## ${++section}. Remediation status`, '');
    p('| ID | Finding | Status | Latest note |', '| --- | --- | --- | --- |');
    for (const f of model.resolved) {
      const last = (f.triage ?? []).slice(-1)[0];
      p(`| ${f.id} | ${escapeMd(f.title)} | ${f.status} | ${escapeMd(truncate(last?.note || '', 120))} |`);
    }
    p('');
    p('Confirm each `fixed` item stays closed with `finding.mjs diff <previous-scan> <this-scan>`; a `retest-pending` item still needs verification before it can be called resolved.', '');
  }
  ...
  p('### Tooling', '');
  if (model.tools.length) {
    p('Findings in this report were raised or corroborated by:', '');
    for (const t of [...model.tools].sort()) p(`- ${escapeMd(t)}`);
    p('');
  } else {
    p('No third-party scanner output was imported; the findings are from first-party analysis and the deterministic surface mapper.', '');
  }
  p('### Risk methodology', '');
  p("Severity follows CVSS — v3.1 to the FIRST specification, v4.0 via FIRST's MacroVector tables. Remediation **priority** additionally fuses EPSS exploit probability, CISA KEV membership and reachability, so a medium under active exploitation is worked before a high in unreachable code. A finding that could not be corroborated is recorded as *needs-validation* with no severity, never padded into the counts.", '');
...
/**
 * OpenVEX 0.2.0. Turns our reachability signal into a portable, standards-based
 * statement about each CVE — the artefact that lets a downstream consumer (Trivy
 * consumes OpenVEX) suppress a "vulnerable but not reachable" alert with a
 * justification. No other orchestrator emits this, and it is exactly what the
 * reachability analysis is for.
 */
function renderVex(model) {
  const now = model.meta.generatedAt;
  const statements = [];
  const productOf = (f) => (f.location?.package ? `pkg:generic/${encodeURIComponent(f.location.package)}`
    : f.location?.file ? `file://${f.location.file}`
      : 'urn:product:audited-component');
  const push = (f, status, extra = {}) => {
    for (const cve of (f.cve ?? [])) {
      statements.push({ vulnerability: { name: cve }, products: [{ '@id': productOf(f) }], status, ...extra });
    }
  };
  for (const f of model.findings) {
    if (f.reachable === false) {
      push(f, 'not_affected', {
        ...
        impact_statement: 'The vulnerable code path is not reachable from an entry point in this deployment.',
      });
    } else {
      push(f, 'affected', { action_statement: f.remediation?.summary || 'See the audit report for remediation.' });
    }
  }
  for (const f of model.needsValidation) {
    push(f, 'under_investigation', { impact_statement: (f.blockers ?? []).join('; ') || 'Reachability or impact pending an external fact.' });
  }
  for (const f of model.resolved) {
    if (f.status === 'fixed') push(f, 'fixed');
  }
  return {
    '@context': 'https://openvex.dev/ns/v0.2.0',
    '@id': `https://openvex.dev/docs/claude-security-audit/${now.replace(/[^0-9]/g, '').slice(0, 14)}`,
    author: model.meta.auditor || 'claude-security-audit',
    role: 'Security auditor',
    timestamp: now,
    version: 1,
    tooling: 'https://github.com/TobiasVeiga00/claude-security-audit',
    statements,
  };
}
...
function main() {
  ...
  for (const format of formats) {
    switch (format) {
      ...
      case 'vex': case 'openvex': {
        const file = `${outBase}.openvex.json`;
        fs.writeFileSync(file, JSON.stringify(renderVex(model), null, 2), 'utf8');
        written.push(file);
        break;
      }
      default:
...
export { buildModel, renderMarkdown, renderHtml, renderSarif, renderCsv, renderVex, riskPosture };
