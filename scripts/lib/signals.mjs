...
export const PATH_SIGNALS = [
  ...
  { id: 'input', re: /(^|[\/_.-])(upload|uploads|import|parser|parse|deserialize|unmarshal|decode|multipart|form|forms|validator|validation|sanitiz)([\/_.-]|$)/i, weight: 22, tags: ['input'] },
  ...
  { id: 'admin', re: /(^|[\/_.-])(admin|superuser|root|internal|debug|dev-?tools?|backdoor|impersonat)([\/_.-]|$)/i, weight: 24, tags: ['privileged'] },
  ...
  { id: 'generated', re: /(^|[\/_.-])(generated|gen|autogen|\.pb\.|_pb2|\.g\.|\.min\.|bundle|vendor|third[_-]?party|migrations?\/\d)/i, weight: -24, tags: ['generated'] },
...
export const SINK_SIGNALS = {
  javascript: [
    ...
    { id: 'js.sqli', re: /\.(query|raw|execute)\s*\(\s*[`'"][^`'"]*(SELECT|INSERT|UPDATE|DELETE|DROP|UNION)\b[^`'"]*[`'"]\s*\+|\.(query|raw)\s*\(\s*`[^`]*\$\{/i, weight: 34, cwe: 'CWE-89', hint: 'string-built SQL' },
  ...
  python: [
    { id: 'py.eval', re: /\b(eval|exec|compile)\s*\(|\b__import__\s*\(/, weight: 34, cwe: 'CWE-95', hint: 'dynamic code execution' },
    ...
    { id: 'py.sqli', re: /(execute|executemany|raw|extra)\s*\(\s*[f]?['"][^'"]*(SELECT|INSERT|UPDATE|DELETE|DROP)\b[^'"]*['"]\s*%|(execute|raw)\s*\(\s*f['"][^'"]*\{|(execute|raw)\s*\([^)]*\+\s*\w+/i, weight: 34, cwe: 'CWE-89', hint: 'string-built SQL' },
  ...
  go: [
    ...
    { id: 'go.error-ignored', re: /\b_\s*[,)]\s*=\s*\w+\.(Verify|Validate|Check|Authenticate|Authorize)/, weight: 24, cwe: 'CWE-252', hint: 'security check result discarded' },
  ],
  ...
  ruby: [
    ...
    { id: 'rb.sqli', re: /(where|find_by_sql|execute|order|group|having|joins|select)\s*\(\s*["'][^"']*#\{|["'][^"']*#\{[^}]*\}[^"']*["']\s*\)\s*(?=[\s.;])/i, weight: 32, cwe: 'CWE-89', hint: 'interpolated SQL fragment' },
  ...
  c: [
    ...
    { id: 'c.memory', re: /\b(alloca|memcpy|memmove|strncpy|strncat)\s*\(|\bfree\s*\([^)]*\)[\s\S]{0,60}\1/, weight: 24, cwe: 'CWE-787', hint: 'manual memory operation to review' },
  ...
  yaml: [
    ...
    { id: 'gha.unpinned', re: /uses\s*:\s*[\w-]+\/[\w.-]+@(main|master|v?\d+)\s*$/m, weight: 16, cwe: 'CWE-1357', hint: 'action not pinned to a commit SHA' },
  ...
  dockerfile: [
    ...
    { id: 'docker.latest', re: /^FROM\s+\S+:latest|^FROM\s+[^:@\s]+\s*$/im, weight: 18, cwe: 'CWE-1104', hint: 'base image not pinned' },
...
export const SECRET_PATTERNS = [
  ...
  { id: 'openai-key', re: /\b(sk-(?:proj-)?[A-Za-z0-9_-]{32,})\b/g, severity: 'critical', provider: 'OpenAI', cwe: 'CWE-798' },
